// src/components/ui/DarkAdminLayout.tsx - Dark sidebar with orange accent branding
import { Inter } from 'next/font/google'
import Link from 'next/link'
import { useRouter } from 'next/router'
import { Button } from '@/components/ui/button'
import { Building2, Users, Phone, BarChart3, Settings, Search, PlusCircle, FileText, ChevronDown, Bell, User } from 'lucide-react'
import { ReactNode } from 'react'

const inter = Inter({ subsets: ['latin'] })

interface DarkAdminLayoutProps {
  children: ReactNode
}

export default function DarkAdminLayout({ children }: DarkAdminLayoutProps) {
  ...
  return (
    <div className={`min-h-screen flex bg-white ${inter.className}`}>
      {/* Sidebar Navigation */}
      <aside className="w-72 bg-gray-900 flex-shrink-0 h-screen sticky top-0 flex flex-col">
        {/* Logo */}
        <div className="p-6">
          <Link href="/admin" className="flex items-center space-x-3">
            ...
            <span className="font-semibold text-lg text-white">Worker Check-in</span>
          </Link>
        </div>
